...
const TOUR_STATUS_CONFIG = {
  ...
  failed:    { label:"Tour Issue",         color:"text-red-600 dark:text-red-400",         bg:"bg-red-50 dark:bg-red-400/10",        icon:"⚠️" },
};
...
// ── Inline cancel confirmation — no native confirm() / alert() ────────────────
function CancelConfirmDialog({ onConfirm, onCancel, loading, error }) {
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-sm
                      border border-gray-200 dark:border-slate-700 overflow-hidden">
        <div className="px-5 pt-5 pb-4">
          <p className="font-bold text-gray-900 dark:text-white mb-1">Cancel booking?</p>
          <p className="text-sm text-gray-500 dark:text-slate-400">This action cannot be undone.</p>

          {/* Error only appears after a failed API call, never on first open */}
          {error && (
            <div className="mt-3 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30
                            rounded-xl px-3 py-2 text-sm text-red-600 dark:text-red-400">
              {error}
            </div>
          )}
        </div>
        ...
        <div className="flex border-t border-gray-100 dark:border-slate-800">
          <button
            onClick={onCancel}
            disabled={loading}
            className="flex-1 py-3.5 text-sm font-medium text-gray-600 dark:text-slate-400
...
export default function UserBookingDetailPage() {
  ...
  // Real-time updates — wrapped in try/catch so malformed payloads never crash
  useSocket("booking:status", useCallback((payload) => {
    try {
      const { orderId, status } = payload ?? {};
      if (orderId?.toString() === id && status) {
        setOrder(prev => prev ? { ...prev, status } : prev);
      }
    } catch (_) {}
  }, [id]));

  useSocket("payment:success", useCallback((payload) => {
    try {
  ...
  const handleCancelClick = () => {
    setCancelError("");     // clear before opening so dialog starts clean
    setShowConfirm(true);
  };
  ...
  const handleCancelConfirm = async () => {
    ...
    try {
      ...
      if (typeof e?.response?.data?.message === "string") msg = e.response.data.message;
        ...
        <div className="max-w-lg mx-auto px-5 pt-5">
          <button
            ...
            className="inline-flex items-center gap-1.5 text-xs font-medium
                       text-gray-500 dark:text-slate-400 bg-white dark:bg-slate-800
                       border border-gray-200 dark:border-slate-700 px-3 py-1.5 rounded-full shadow-sm
        ...
        <div className="max-w-lg mx-auto px-5 py-4 space-y-4">
          ...
          {ts && (
            <div className={`rounded-2xl px-5 py-3 flex items-center gap-3 ${ts.bg}`}>
              ...
              <div className="flex-1">
                ...
                {order.tourStartedAt && (
                  <p className="text-xs text-gray-400 dark:text-slate-500 mt-0.5">Started: {fmtDate(order.tourStartedAt)}</p>
                )}
                {order.tourCompletedAt && (
                  <p className="text-xs text-gray-400 dark:text-slate-500 mt-0.5">Completed: {fmtDate(order.tourCompletedAt)}</p>
                )}
                {order.tourStatus === "failed" && order.tourFailReason && (
                  <p className="text-xs text-red-500 dark:text-red-400 mt-0.5">Reason: {order.tourFailReason}</p>
                )}
              </div>
            </div>
          )}
          ...
          {order.status === "holding" && secsLeft === 0 && (
            <div className="bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 rounded-2xl px-4 py-3">
              <p className="text-sm text-red-600 dark:text-red-400">
                ⏰ Payment window expired. This booking will be cancelled shortly.
              </p>
            </div>
          )}

          {/* Location / Tour header */}
            ...
            {img ? (
              ...
              <div className="w-full h-44 bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-800
                ...
                <p className="text-white font-bold text-base text-center px-6 relative z-10 line-clamp-2 drop-shadow">
                  {order.tourProductDetails?.tourProduct?.title || d?.tourProduct?.title || "Tour Package"}
                </p>
              </div>
            ) : null}
            <div className="p-4">
          ...
          <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl p-4">
            <p className="text-xs font-semibold text-gray-400 dark:text-slate-500 uppercase tracking-wider mb-3">
              {t === "tour_product" ? "Tour Details" : t === "hotel" ? "Room Booking Details"
               : t === "restaurant" ? "Table Booking Details" : t === "entertainment" ? "Ticket Details" : "Booking Details"}
            </p>
            {t === "restaurant" && <>
