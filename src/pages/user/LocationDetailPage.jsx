        ...
        <svg
          ...
          className={`${dim} ${
            i <= rating
              ...
              : "text-gray-200 dark:text-slate-700 fill-current"
          }`}
          viewBox="0 0 24 24"
        >
...
// ─── Map Modal ────────────────────────────────────────────────────────────────
// z-[70] — above BookingModal (z-[60]) and Leaflet tiles
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl overflow-hidden w-full max-w-2xl shadow-2xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-slate-800">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={onClose}
              className="w-8 h-8 rounded-full flex items-center justify-center text-lg text-gray-400
                         hover:text-gray-700 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-slate-800 transition-colors"
            >
              ×
            </button>
          </div>
        ...
        <div style={{ height: 400 }}>
          <MapView
            ...
            height="100%"
            showUserBtn
            markers={[{ lat, lng, title: location.name, description: location.address?.full }]}
          />
        </div>
      </div>
    </div>
...
function HotelAmenitiesSection({ amenities }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl p-5">
      <h2 className="font-bold text-gray-900 dark:text-white mb-4">What this place offers</h2>
      ...
      {amenities.length > 10 && (
        <button
          ...
          className="mt-4 text-sm font-semibold text-gray-900 dark:text-white underline underline-offset-2 hover:text-gray-600"
        >
...
function RoomsSection({ locationId, onBook }) {
  ...
  useEffect(() => {
    api
      .get("/units/public", { params: { locationId, unitType: "room", includeInactive: "false" } })
      .then((r) => {
        const units = r.data.data ?? [];
        const grouped = {};
        units.forEach((u) => {
          const key = u.roomType || "standard";
          if (!grouped[key]) grouped[key] = { ...u, totalCount: 0, availableCount: 0 };
  ...
  if (loading)
    return (
      <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl p-5 animate-pulse">
        <div className="h-5 bg-gray-200 dark:bg-slate-700 rounded w-40 mb-4" />
        {[1, 2].map((i) => (
          <div key={i} className="h-36 bg-gray-100 dark:bg-slate-800 rounded-xl mb-3" />
        ))}
      </div>
    );
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl overflow-hidden">
      ...
      {rooms.map((room, i) => {
        ...
        const discount  =
          room.originalPrice > 0 && room.originalPrice > room.pricePerUnit
        ...
        return (
          <div
            key={i}
            className={`flex flex-col sm:flex-row ${i > 0 ? "border-t border-gray-100 dark:border-slate-800" : ""}`}
          >
            {/* Room image */}
            <div className="p-4 border-b sm:border-b-0 sm:border-r border-gray-100 dark:border-slate-800">
              {img ? (
                <img src={img} alt={room.roomType} className="w-full h-28 sm:h-32 object-cover rounded-xl mb-3" />
              ) : (
                <div className="w-full h-28 sm:h-32 bg-gray-100 dark:bg-slate-800 rounded-xl mb-3 flex items-center justify-center text-3xl">
                  🛏️
                </div>
              )}
              <p className="font-semibold text-gray-900 dark:text-white text-sm capitalize">{room.roomType} room</p>
              {room.roomSize && <p className="text-xs text-gray-400 dark:text-slate-500 mt-1">📐 {room.roomSize} m²</p>}
              {BED_LABEL[room.bedType] && <p className="text-xs text-gray-500 dark:text-slate-400 mt-1">🛏 {BED_LABEL[room.bedType]}</p>}
              {VIEW_LABEL[room.viewType] && <p className="text-xs text-gray-500 dark:text-slate-400 mt-1">🪟 {VIEW_LABEL[room.viewType]}</p>}
            ...
            <div className="flex-1 p-4 sm:p-5">
              ...
              <div className="space-y-3">
                    ...
                    <div>
                      {discount && room.originalPrice > 0 && (
                        <div className="flex items-center gap-2 mb-0.5">
                          ...
                          <span className="text-xs font-bold text-orange-500 bg-orange-100 dark:bg-orange-500/15 px-1.5 py-0.5 rounded">
                            -{discount}%
                          </span>
                        </div>
                      )}
                ...
                {!room.includesBreakfast && (
                  <div className="border border-gray-100 dark:border-slate-800 rounded-xl p-3 bg-emerald-50/40 dark:bg-emerald-500/5">
                    <p className="text-xs text-gray-400 dark:text-slate-500 mb-1">
                      {room.roomType?.charAt(0).toUpperCase() + room.roomType?.slice(1)} — With breakfast
                    </p>
                    <p className="text-sm font-semibold text-gray-900 dark:text-white mb-2">Breakfast for 2 guests</p>
                    <div className="space-y-1 mb-3">
                      <p className="text-xs text-emerald-600 dark:text-emerald-400 flex items-center gap-1.5">
                        <span>🍳</span> Breakfast included
                      </p>
...
function PolicyCard({ location }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl p-5">
      <h3 className="font-bold text-gray-900 dark:text-white mb-3 text-sm">House rules</h3>
      <div className="space-y-2">
        ...
        {pol.customNotes && (
          <p className="text-xs text-gray-500 dark:text-slate-400 mt-2 pt-2 border-t border-gray-100 dark:border-slate-800">
            {pol.customNotes}
          </p>
        )}
      </div>
...
export default function LocationDetailPage() {
  ...
  // FIX: selectedRoomType resets when BookingModal closes
  ...
  // Ref to scroll to rooms section
  const roomsSectionRef = useRef(null);

  // ── Fetch location ──────────────────────────────────────────────────────
  useEffect(() => {
    // Reset per-location state when navigating between locations
    setLocation(null);
    setLoading(true);
    setActiveImg(0);
    setDistanceKm(null);
    ...
    api
      ...
      .then((r) => {
        ...
        if (user) {
          api
            .get("/bookings/my", { params: { limit: 50 } })
            .then((res) => {
              const active = res.data.data.orders?.find(
                (o) =>
  ...
  const handleFavorite = async () => {
    ...
    if (result.isFavorite) trackSave(id, location?.category, location?.priceRange?.label);
    else trackUnsave(id);
  };

  // ── Handle open booking ─────────────────────────────────────────────────
  // Called from header "Book Now" button — no prefilled room type
  const handleOpenBook = () => {
    setSelectedRoomType(null);
    setShowBook(true);
  };

  // Called from RoomsSection — with room type prefilled
  const handleBookRoom = (roomType) => {
    setSelectedRoomType(roomType || null);
    setShowBook(true);
  };

  // FIX: reset selectedRoomType when modal closes so next open is clean
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-950">
        ...
        <div className="flex items-center justify-between mb-4 gap-3">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            {location?.booking?.isBookable === true && user && (
              userBooking ? (
                // Already booked → view existing booking
                <button
                  onClick={() => navigate(`/profile/bookings/${userBooking._id}`)}
                  className="flex items-center gap-1.5 px-4 py-1.5 rounded-xl text-sm font-semibold
                             bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700
                             text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-700 transition-all"
                >
                  📋 View booking
                </button>
              ) : (
                // Not booked yet → scroll down to rooms section
                <button
                  onClick={() => roomsSectionRef.current?.scrollIntoView({ behavior: "smooth", block: "start" })}
                  className="flex items-center gap-1.5 px-4 py-1.5 rounded-xl text-sm font-semibold
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="rounded-2xl overflow-hidden">
              {images.length === 0 ? (
                <div className="h-72 bg-gray-200 dark:bg-slate-800 flex items-center justify-center text-5xl">
                  🏞️
                </div>
              ) : (
                <div>
            ...
            {/* Scroll anchor for non-hotel bookable locations */}
            {location.booking?.isBookable && location.category !== "hotel" && (
              <div ref={roomsSectionRef} className="-mt-2">
                <button
                  onClick={handleOpenBook}
                  className="w-full py-3.5 bg-blue-600 hover:bg-blue-700 text-white font-semibold
                             rounded-2xl text-sm transition-colors flex items-center justify-center gap-2"
                >
                  📅 Book this place
                </button>
              </div>
            ...
            {location.hotelInfo?.highlights?.length > 0 && (
              <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl p-5">
                <h2 className="font-bold text-gray-900 dark:text-white mb-4">Why guests love it</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  {location.hotelInfo.highlights.map((h, i) => (
                    <div key={i} className="flex items-center gap-2.5">
                      <div className="w-5 h-5 rounded-full bg-teal-100 dark:bg-teal-500/20 flex items-center justify-center shrink-0">
                        <svg className="w-3 h-3 text-teal-600 dark:text-teal-400" fill="none" stroke="currentColor" strokeWidth={3} viewBox="0 0 24 24">
            ...
            {location.category === "hotel" && location.booking?.isBookable && (
              <div ref={roomsSectionRef}>
                <RoomsSection locationId={location._id} onBook={handleBookRoom} />
              </div>
            )}
                  ...
                  {atReviews.map((r, i) => (
                    <div key={i} className="bg-gray-50 dark:bg-slate-800 rounded-xl p-4">
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-full bg-teal-100 dark:bg-teal-600/20 flex items-center justify-center text-teal-600 dark:text-teal-400 font-bold text-sm shrink-0">
                      ...
                      {r.tags?.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-2">
                          {r.tags.map((t) => (
                            <span key={t} className="text-[10px] bg-teal-100 dark:bg-teal-500/20 text-teal-700 dark:text-teal-400 px-2 py-0.5 rounded-full">
                              {t}
                            </span>
                          ))}
                        </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                <select
                  ...
                  className="text-xs bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700
                             text-gray-600 dark:text-slate-300 rounded-lg px-2 py-1.5 focus:outline-none transition-all"
                >
              ...
              {/* Rating summary */}
              {revSummary?.total > 0 && (
                <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl p-4 mb-4 flex items-center gap-6">
              ...
              {user && !reviewed && !showReviewForm && (
                <button
                  ...
                  className="w-full mb-4 py-3 rounded-xl border-2 border-dashed border-gray-300 dark:border-slate-700
                             ...
                             hover:border-blue-400 hover:text-blue-600 dark:hover:border-blue-500 dark:hover:text-blue-400 transition-all"
                >
                  ✍️ Write a review
                </button>
              )}
              ...
              {/* Review form */}
              {showReviewForm && (
                <div className="mb-5 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl p-5 space-y-4">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={4}
                      placeholder="Share your experience, tips, what you liked..."
              ...
              {revLoading && reviews.length === 0 ? (
                <div className="space-y-3">
                  {[1, 2, 3].map((i) => (
                    <div key={i} className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-4 animate-pulse space-y-2">
                ...
                <div className="space-y-4">
                  {reviews.map((r, i) => (
                    <div key={i} className="flex gap-3">
                      <div className="w-9 h-9 rounded-full bg-gray-200 dark:bg-slate-700 flex items-center justify-center text-gray-500 dark:text-slate-400 font-semibold text-sm shrink-0">
                    ...
                    return (
                      <div className="flex gap-3">
                        ...
                        <div>
                          ...
                          <span className={`text-xs font-semibold ${isOpenNow ? "text-emerald-600 dark:text-emerald-400" : "text-red-500"}`}>
                ...
                {location.contact?.phone && (
                  <div className="flex gap-3">
                    ...
                    <div>
                      ...
                      <a href={`tel:${location.contact.phone}`} className="text-sm text-blue-600 dark:text-blue-400 hover:underline">
                        {location.contact.phone}
                      </a>
                    </div>
                  </div>
                  ...
                  <div className="mt-4">
                    {distanceKm !== null && (
                      <div className="flex items-center gap-1.5 mb-2 text-xs text-blue-600 dark:text-blue-400 font-medium">
                        <span>📍</span>
                        <span>{fmtDistance(distanceKm)} from you</span>
                      </div>
                    )}
                    {/*
                      FIX: wrap map in relative+isolate container to prevent Leaflet's
                      internal z-index from bleeding above the modal overlay.
                      The BookingModal uses z-[60] and MapModal uses z-[70].
                    */}
                    <div className="rounded-xl overflow-hidden border border-gray-200 dark:border-slate-700 h-52 relative isolate">
                      {hasCoords ? (
                        <MapView
                          center={{ lat, lng }}
                          zoom={15}
                        ...
                        <div className="w-full h-full bg-gray-100 dark:bg-slate-800 flex flex-col items-center justify-center text-gray-400 dark:text-slate-500">
                          <span className="text-3xl mb-1">🗺️</span>
                          <p className="text-xs">No coordinates available</p>
                        </div>
                      )}
                    </div>
                  </div>
      ...
      {showMap && <MapModal location={location} onClose={() => setShowMap(false)} />}
      ...
      {/* FIX: use handleCloseBook so selectedRoomType resets on close */}
      <BookingModal
        open={showBook}
        location={location}
