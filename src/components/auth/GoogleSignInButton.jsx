...
export default function GoogleSignInButton({ onSuccess, label = "Continue with Google" }) {
  ...
  const handleClick = () => {
    ...
    window.google.accounts.id.prompt((notification) => {
      if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
        ...
        const container = document.getElementById("__gsi_hidden__");
        if (container) {
  ...
  return (
    <div>
      {/* Hidden GSI container for fallback */}
      <div id="__gsi_hidden__" style={{ display:"none" }}/>
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-3 px-4 py-3
                   ...
                   rounded-xl text-sm font-medium text-gray-700 dark:text-slate-200
