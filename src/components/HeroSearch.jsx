...
import {
  Hotel, UtensilsCrossed, Landmark, Coffee,
  Sparkles, Map
} from "lucide-react";

const CATEGORIES = [
  { key: "hotel",         label: "Hotels",       icon: <Hotel size={16} strokeWidth={1.5}/> },
  { key: "restaurant",    label: "Restaurants",  icon: <UtensilsCrossed size={16} strokeWidth={1.5}/> },
  { key: "tourist_spot",  label: "Sightseeing",  icon: <Landmark size={16} strokeWidth={1.5}/> },
  { key: "cafe",          label: "Cafes",        icon: <Coffee size={16} strokeWidth={1.5}/> },
  { key: "entertainment", label: "Entertainment",icon: <Sparkles size={16} strokeWidth={1.5}/> },
  { key: "tour",          label: "Tours",        icon: <Map size={16} strokeWidth={1.5}/> },
];
...
function GuestCounter({ value, onChange }) {
  ...
  const label    = `${adults + children} people`;
...
export default function HeroSearch({ mode = "location" }) {
  ...
  return (
    <div className="mb-8">
      {/* Hero container — be warm + dot pattern via inline div (no before: to avoid blocking clicks) */}
      <div className="relative rounded-3xl bg-blue-600 dark:bg-blue-950 px-4 sm:px-6 pt-6 pb-6 transition-colors">
          ...
          {!isTourMode && (
            <div className="mb-4">
              ...
              <p className="text-blue-200 text-xs sm:text-sm mt-1">
                Explore thousands of amazing places across Vietnam
              </p>
            </div>
          )}
              ...
              {CATEGORIES.map(({ key, label, icon }) => (
                <button key={key} type="button"
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs sm:text-[13px]
                              font-medium transition-all shrink-0 border
                              ${category === key
              ...
              <div className="flex-1 min-w-0 relative">
                <div className="flex items-center gap-2 px-4 py-3">
                  ...
                  <input ref={inputRef} value={destination}
                    ...
                    placeholder="Location, hotel, restaurant..."
